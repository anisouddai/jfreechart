...
public abstract class AbstractXYItemRenderer extends AbstractRenderer
    ...
    public void drawDomainMarker(Graphics2D g2, XYPlot plot, 
        ...
        if (marker instanceof ValueMarker) {
            ...
            if (im.getOutlinePaint() != null && im.getOutlineStroke() != null) {
                drawOutlines(orientation, g2, dataArea, im, range, start2d, end2d);
            }
    ...
    private void drawOutlines(PlotOrientation orient, Graphics2D g2, Rectangle2D dataArea, IntervalMarker intervalMarker, Range range,
        ...
        if (range.contains(intervalMarker.getStartValue())) {
            if (orient == PlotOrientation.VERTICAL) {
                line.setLine(start2d, y0, start2d, y1);
            } else {
                line.setLine(y0, start2d, y1, start2d);
            }

            g2.draw(line);
        }
        if (range.contains(intervalMarker.getEndValue())) {
            if (orient == PlotOrientation.VERTICAL) {
                line.setLine(end2d, y0, end2d, y1);
            }else{
                line.setLine(y0, end2d, y1, end2d);
            }
            g2.draw(line);
        }
    }
